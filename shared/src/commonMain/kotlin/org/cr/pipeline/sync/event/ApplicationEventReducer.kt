...
fun applyEvent(state: ApplicationState?, event: ApplicationEvent, today: LocalDate): ApplicationState = when (event) {
    is ApplicationCreated -> ApplicationState(
        ...
        statusHistory = listOf(
            StatusHistoryRecord(event.input.status, event.input.dateApplied ?: today, "Application created", event.provenance),
        ),
        contacts = emptyList(),
        reminders = emptyList(),
    ...
    is ApplicationEdited -> {
        ...
        current.copy(
            ...
            statusHistory = if (current.status != event.input.status) {
                current.statusHistory + StatusHistoryRecord(event.input.status, today, "Updated via edit", event.provenance)
            } else {
    ...
    is StatusChanged -> {
        ...
        current.copy(
            status = event.status,
            statusHistory = current.statusHistory + StatusHistoryRecord(event.status, today, event.note, event.provenance),
        )
    }
