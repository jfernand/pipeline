...
#feature(
  ...
  summary: [
    ...
    application's detail screen, `pipeline://app/{id}/status` for the same with the Update status
    sheet open, `pipeline://app/new` and `pipeline://app/{id}/edit` for the Add
    and Edit forms, and
  ...
  implementation: (
    ...
    "shared/src/commonMain/kotlin/org/cr/pipeline/ui/phone/PipelinePhoneApp.kt, ui/PipelineApp.kt — navDeepLink<DetailRoute>(basePath = \"pipeline://app\"), navDeepLink<ListRoute>(basePath = \"pipeline://list\"), navDeepLink { uriPattern = \"pipeline://app/new\" } and \"pipeline://app/{id}/edit\" on AddEditRoute, \"pipeline://app/{id}/status\" on UpdateStatusRoute (redirects to DetailRoute with the sheet open)",
